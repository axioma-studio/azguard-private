...
| **Custom (runtime) roles** | ✅ DB-backed `AzRole` | ✅ Yes | ✅ Yes | ✅ Yes |
...
| **Filament v3+ support** | ✅ First-party package | ⚠️ Community plugin | ❌ No | ❌ No |
...
| **Built-in doctor** | ✅ `azguard:doctor` | ❌ No | ❌ No | ❌ No |
| **Pest test helpers** | ✅ Yes | ❌ No | ❌ No | ❌ No |
...
4. **Filament-native** — `azguard/filament` provides `GuardResource`, `RoleResource`, and `DirectGrantResource` out of the box.
5. **Octane-safe** — no static state, per-request cache keyed by user ID.
6. **Doctor command** — built-in `azguard:doctor` catches misconfigurations before they reach production.
