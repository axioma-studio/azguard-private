...
This guide covers how to seed roles and permissions into your application — both static (code-defined) roles and dynamic (DB-backed) roles.
...
```php
// In a seeder, factory, or anywhere
...
Always use `firstOrCreate` / `updateOrCreate` so seeders can run multiple times without duplicating data:
...
// Bad — creates duplicates on re-seed
