...
`artisan guard:doctor` сканирует конфиг, миграции и определения ролей и сообщает о несоответствиях до продакшена.
...
```php [1. Определите]
// Значения без префикса; панель добавляет префикс 'app.documents.*'.
...
// Зарегистрируйте enum в панели (в вашем PanelProvider):
